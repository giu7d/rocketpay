defmodule Rocketpay.Accounts.Deposit do
  ...
  def execute(%{"id" => id, "value" => value}) do
    Multi.new()
    |> Multi.run(:account, fn repo, _changes -> get_account(repo, id) end)
    |> Multi.run(:update_balance, fn repo, account ->
  ...
  defp update_balance(repo, %{account: account}, value) do
    account
    |> sum_values(value)
    |> update_account(repo, account)
  end

  defp sum_values(%Schema.Account{balance: balance}, value) do
    value
    |> Decimal.cast()
    |> handle_cast(balance)
  end
